...
if __name__ == "__main__":
    ...
    # iterate samples and binomial and beta-binomial clusting output to format input for clonevol
    for sample in sample_info:
        for model in ["binomial_clonal", "beta_binomial_clonal"]:
            ...
            with open(sample[model], "rt") as cmfp:
                reader = csv.DictReader(cmfp, delimiter="\t")
                for row in reader:
                    mut = row["mutation_id"]
                    ...
                    variant_data[mut] = {
                        ...
                        "ccf": row["cellular_prevalence"],
                        "is_interesting": "FALSE",
                        "gene": "",
                    }

            # add variant allele fraction (VAF) for variants for each sample
            with open(sample["pyclone_input_vars"], "rt") as cmfp:
                reader = csv.DictReader(cmfp, delimiter="\t")
                for row in reader:
                    mut = row["mutation_id"]
            ...
            with open(ARGS.neatvars, "rt") as cmfp:
                ...
                for row in reader:
                    ...
                    if (
                        row["Accession"] == sample["sample_id"]
                        and mut_id in variant_data
                    ):
            ...
            for variant in variant_data.values():
                variant["cluster"] = new_clust_labels[variant["ccf"]]

            # generate output file for the given model
            output_file = (
                Path(ARGS.output) / f"{sample['sample_id']}-{model}-clonevol.tsv"
            )
